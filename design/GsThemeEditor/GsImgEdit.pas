...
uses
  ...
  Classes, Dialogs, ComCtrls, ImgList, ExtDlgs, RzPanel, RzSplit, RzListVw,
  RzLabel, RzRadGrp, RzButton;
...
type
  ...
  public
    constructor Create(AOwner: TList; AOwnerForm: TImageListEditor;
    ...
    property CanChangeTransparent: Boolean read FCanChangeTransparent
      write SetCanChangeTransparent;
    property CanChangeFill: Boolean read FCanChangeFill write SetCanChangeFill;
    property CanChangeOperation: Boolean read FCanChangeOperation
      write SetCanChangeOperation;

  end;

  TImageListEditor = class(TFrame)
    // TImageListEditor = class(TFrame)
    OK: TButton;
    Cancel: TButton;
    Apply: TButton;
    ...
    ImageListGroup: TRzGroupBox;
    ImageView: TRzListView;
    Add: TButton;
    Delete: TButton;
    Clear: TButton;
    ReplaceBtn: TButton;
    ImageGroup: TRzGroupBox;
    TransparentLabel: TRzLabel;
    FillLabel: TRzLabel;
    OptionsGroup: TRzRadioGroup;
    MainPanel: TRzPanel;
    MainImage: TImage;
    OptionsPanel: TRzPanel;
    TransparentColor: TColorBox;
    FillColor: TColorBox;
    RzSplitter1: TRzSplitter;
    ActionImages: TImageList;
    RzToolbar1: TRzToolbar;
    TBSaveAll: TRzToolButton;
    TBDelete: TRzToolButton;
    TBClear: TRzToolButton;
    TBRefresh: TRzToolButton;
    TBUp: TRzToolButton;
    TBDown: TRzToolButton;
    RzSpacer1: TRzSpacer;
    RzSpacer2: TRzSpacer;
    RzSpacer3: TRzSpacer;
    TBExport: TRzToolButton;
    TBNew: TRzToolButton;
    TBOpen: TRzToolButton;
    RzGroupBox1: TRzGroupBox;
    FileView: TRzListView;
  ...
  private
    ...
    function DoAdd(Graphic: TGraphic; Index: Integer; DivX, DivY: Integer)
      : Integer; overload;

    procedure Replace(Index: Integer; Graphic: TGraphic;
...
uses ToolsAPI, CommCtrl, TypInfo, DsnConst, pngimage, GIFImg;

resourcestring
  SImageTypeTransparencyNotImplemented =
    'Die Transparenz für den Bildtyp %s ist nicht implementiert und wird nicht richtig angezeigt.';
  (*
    SImageListTooBig = 'Maximale Bildgrösse von 256x256 ist überschritten.';
    SImageListEditorCaption = '%s%s%s ImageList';
    sNoUnusedColors = 'Keine unbenutzten Farben gefunden.';
    SAddImagesTitle = 'Bilder hinzufügen';
    SImageListDivide = 'Die Dimensionen für Bitmap %s sind grösser als die der Bilderliste. In %d verschiedene Bitmaps aufteilen?';
  *)
...
procedure StretchImageList(SrcList, DstList: TImageList;
  ...
  with ListItems do
    try
      ...
      try
        ...
        Caption := Format(SImageListEditorCaption,
          [AImageList.Owner.Name, DotSep, AImageList.Name]);
...
function TImageListEditor.GetActiveControl: TWinControl;
begin
  Result := GetParentForm.ActiveControl;
end;
...
function TImageListEditor.GetParentForm: TForm;
var
  Ctrl: TWinControl;
begin
  Ctrl := Parent;
  while (Ctrl <> nil) and not (Ctrl is TForm) do
    Ctrl := Ctrl.Parent;

  if (Ctrl <> nil) and (Ctrl is TForm) then
    Result := Ctrl as TForm
  else
    Result := nil;
end;

procedure TImageListEditor.SetActiveControl(const Value: TWinControl);
begin
  GetParentForm.ActiveControl := Value;
end;
...
begin
  ...
  try
    ...
    MainImage.Picture.Bitmap.Canvas.FillRect(Rect(0, 0, FImages.Width,
      FImages.Height));
...
function TImageListEditor.DoAdd(Graphic: TGraphic;
  Index, DivX, DivY: Integer): Integer;
var
  Bmp: TBitmap;
  Clip: TBitmap;
  X, Y: Integer;
...
procedure TImageListEditor.ImageViewCompare(Sender: TObject;
  Item1, Item2: TListItem; Data: Integer; var Compare: Integer);
begin
  Compare := StrToIntDef(Item1.Caption, 0) - StrToIntDef(Item2.Caption, 0);
end;

procedure TImageListEditor.ImageViewDragDrop(Sender, Source: TObject;
  X, Y: Integer);
var
  FromIndex, ToIndex: Integer;
  R: TRect;
begin
  if not Assigned(ImageView.ItemFocused) or not Assigned(ImageView.DropTarget)
  then
    Exit;
...
procedure TImageListEditor.ImageViewDragOver(Sender, Source: TObject;
...
procedure TImageListEditor.ImageViewEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  DragTimer.Enabled := False;
end;

procedure TImageListEditor.ImageViewSelectItem(Sender: TObject;
...
begin
  ...
  while FImageInfo.Count > 0 do
    TImageInfo(FImageInfo.Last).Free;
  FImageInfo.Free;
end;
        ...
        begin
          ...
          begin
            if (Picture.Width > IWidth) and (Picture.Width mod IWidth = 0) then
              ...
              XDiv := 1;
            if (Picture.Height > IHeight) and (Picture.Height mod IHeight = 0)
            then
            ...
            if ((XDiv > 1) or (YDiv > 1)) and
              ...
              DialogResult :=
                MessageDlg(Format(SImageListDivide,
...
begin
  ...
  try
    ...
    case Operation of
      ioCrop:
        Bmp.Canvas.StretchDraw(Rect(0, 0, Graphic.Width,
          Graphic.Height), Graphic);
      ioStretch:
        Bmp.Canvas.StretchDraw(Rect(0, 0, FImages.Width,
          FImages.Height), Graphic);
      ioCenter:
        Bmp.Canvas.StretchDraw(Bounds((FImages.Width - Graphic.Width) div 2,
          (FImages.Height - Graphic.Height) div 2, Graphic.Width,
          Graphic.Height), Graphic);
    end;
...
begin
  ...
  begin
    ...
    OptionsPanel.Width := ImageGroup.Left + ImageGroup.Width -
      OptionsPanel.Left - 20;
  end;
...
begin
  ...
  begin
    ...
    if Graphic is TBitmap and (TBitmap(Graphic).AlphaFormat = afIgnored) then
      ...
      FTransparentColor := TPngImage(Graphic).TransparentColor and $FFFFFF
      (*
        else if Graphic is TGIFImage and Graphic.Transparent then
        FTransparentColor := TGIFImage(Graphic).EffectiveBackgroundColor and $FFFFFF
      *)
    else if Graphic.Transparent then
    begin
      MessageDlg(Format(SImageTypeTransparencyNotImplemented,
        [Graphic.ClassName]), mtWarning, [mbOk], 0);
      FTransparentColor := clDefault;
      //Graphic.Canvas.Pixels[0, 15 ];
      //ImageList.AddMasked( Glyph, Glyph.Canvas.Pixels[ 0, 15 ] );
    end
    else
      FTransparentColor := clDefault;

    FCanChangeTransparent :=
      (Graphic is TBitmap and (TBitmap(Graphic).AlphaFormat = afIgnored)) or
    ...
    FCanChangeOperation := (Graphic.Width <> LWidth) or
      (Graphic.Height <> LHeight);
